...
export function ReferralPanel({ userId, onClose }: Props) {
  ...
  useEffect(() => {
    // Ensure the code is persisted server-side (idempotent)
    api.getReferralLink(userId).catch(() => {});

    // Fetch stats separately
    api.getReferralStats(userId)
      .then(res => setStats({ ...res.stats, credits: res.credits }))
      .catch(() => setStats({ total: 0, pending: 0, successful: 0, credits: 0 }))
  ...
  const copyLink = useCallback(() => {
    navigator.clipboard.writeText(referralUrl).then(() => {
      ...
      // Fallback: open share dialog or do nothing — execCommand is deprecated
      setCopied(true);
      setTimeout(() => setCopied(false), 2500);
    });
  }, [referralUrl]);
  ...
  const shareInstagram = useCallback(() => {
    const caption = `I just tested my reel using AI and got insane insights 🔥\n\nTry BoostMyReel free 👇\n${referralUrl}\n\n#reels #instareels #contentcreator #growthhack`;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Referral link — always visible, no API dependency */}
        ...
        <button onClick={shareInstagram} style={{
          ...
          background: igCopied ? '#10b981' : 'linear-gradient(135deg, #833ab4, #fd1d1d, #fcb045)',
          border: 'none', color: 'white', fontSize: 13, fontWeight: 600,
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
        }}>
          {igCopied ? <><Check size={13} /> Copied!</> : 'Copy for Instagram'}
        </button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 20 }}>
          {[
            { label: 'Total',   value: stats?.total      ?? 0, color: '#475569' },
            { label: 'Pending', value: stats?.pending     ?? 0, color: '#f59e0b' },
            { label: 'Earned',  value: stats?.successful  ?? 0, color: '#10b981' },
          ].map(s => (
            <div key={s.label} style={{
              background: '#f8fafc', border: '1px solid #e2e8f0',
              borderRadius: 10, padding: '10px 8px', textAlign: 'center',
            }}>
              <div style={{ fontSize: 20, fontWeight: 800, color: statsLoading ? '#cbd5e1' : s.color }}>
